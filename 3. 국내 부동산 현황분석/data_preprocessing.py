...
# 공통 경로 지정
base_path = '/Users/SOO/Desktop/데분 포트폴리오/Data_Project/3. 국내 부동산 현황분석/국토교통부_부동산 실거래가(수진 수집 완)/'
...
single_family_trade_file = '단독다가구(매매)/단독다가구(매매)_실거래가_'
...
single_family_trade_data_list = [pd.read_csv(base_path + single_family_trade_file + fname, encoding='cp949', skiprows=15) for fname in filenames]
...
file_names = {
    ...
    '단독다가구(매매)_데이터' : single_family_trade_data_list
}
'''
...
for name, db in file_names.items():
    ...
    property_data.to_csv(f'/Users/SOO/Desktop/데분 포트폴리오/Data_Project/3. 국내 부동산 현황분석/국토교통부_부동산 실거래가(수진 수집 완)/취합/(최종) {name}.csv', encoding='euc-kr', index = False)
